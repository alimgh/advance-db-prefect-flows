...
def weather_and_pollution_flow():
    ...
    for city, coords in cities.items():
        ...
        kafka_event["weather_main"] = weather_data["weather"]["main"]
        kafka_event["weather_description"] = weather_data["weather"]["description"]

        # Fetch current air pollution data
        air_pollution_data = get_air_pollution_data(lat, lon)
        # Add city name for clarity
        kafka_event["pol_aqi"] = air_pollution_data["main"]["aqi"]
        kafka_event["pol_co"] = air_pollution_data["components"]["co"]
        kafka_event["pol_no"] = air_pollution_data["components"]["no"]
        kafka_event["pol_no2"] = air_pollution_data["components"]["no2"]
        kafka_event["pol_o3"] = air_pollution_data["components"]["o3"]
        kafka_event["pol_so2"] = air_pollution_data["components"]["so2"]
        kafka_event["pol_pm2_5"] = air_pollution_data["components"]["pm2_5"]
        kafka_event["pol_pm10"] = air_pollution_data["components"]["pm10"]
        kafka_event["pol_nh3"] = air_pollution_data["components"]["nh3"]
